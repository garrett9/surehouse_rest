...
class UsersController extends Controller {
	...
	/**
	 * The middleware will check the validity of the request, and if successfull, a new token will be issued.
	 *
	 * @return Response
	 */
	public function refresh() {
		return self::ok ( 'New token issued!' );
	}
	...
	public function resetPassword($token) {
		...
		$response = $this->passwords->reset(array_merge(Input::only('email', 'password', 'password_confirmation'), ['token' => $token]), function($user, $password) {
	 ...
	 */
	public function all() {
		$users = User::where ( 'permission', '!=', 'super admin' )->get ();
		if (! $users->isEmpty ())
			return self::ok ( null, $users );
		return self::no_content ( 'There are currently no users in the system!' );
	}
