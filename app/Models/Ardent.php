...
abstract class Ardent extends Model {
    ...
    /**
     * CUSTOM Function added by Garrett.
     * Only validate the rules having the keys of the given field names.
     *
     * @param array $fields			Fields to be validated.
     * @param array $customMessages	Custom error messages
     * @return bool
     */
    public function validateOnly(array $fields = array(), array $customMessages = array(), $noUniques=false) {
    	$rules = array();
    	foreach($fields as $field) {
    		if(isset(static::$rules[$field]))
    			$rules[$field] = static::$rules[$field];
    	}
    	return $this->validate($rules, $customMessages, $noUniques);
    }
